...
import { Loader2, RefreshCw, AlertCircle, Hash, ChevronLeft, ChevronRight, X, Layout, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    const fetchData = async () => {
        ...
        try {
            const [quesData, attendData] = await Promise.all([
                matchmakingApi.getExhibitorMatchmakingQuestions(currentId, token),
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <p className="text-sm font-medium text-text-secondary flex items-center gap-2">
                                ...
                                Exhibitor Portal Questions
                                · {questions.length} portal-enabled {questions.length === 1 ? 'question' : 'questions'}
                            </p>
                        </div>
                    </div>
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-32 text-text-tertiary gap-8 animate-pulse">
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-xs font-bold text-text-tertiary uppercase tracking-wider">Exhibitor Portal Questions</p>
                    </div>
                </div>
            ) : error ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-10 items-start">
                    ...
                    {questions.length === 0 && (
                        <div className="col-span-full py-32 px-10 text-center bg-white rounded-[3rem] border-2 border-dashed border-border/60 flex flex-col items-center group hover:border-accent/40 transition-colors duration-500">
                            ...
                            <h3 className="text-2xl font-bold text-text-primary mb-3 tracking-tight">No Portal Questions</h3>
                            <p className="text-xs text-text-secondary max-w-sm mx-auto mb-4 leading-relaxed font-medium">
                                No matchmaking questions are enabled for the exhibitor portal on this event. Enable <strong>Exhibitor Portal</strong> on questions in the Matchmaking Questions tab.
                            </p>
                        </div>
                    )}
                </div>
...
export default ExhibitorPortalQuestions;
