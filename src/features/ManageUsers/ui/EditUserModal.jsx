...
/**
 * Edit profile / password. Permissions are not editable here.
 */
const EditUserModal = ({ isOpen, onClose, user, onSubmit, isSaving, serverError }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const country_code = form.country_code.trim().replace(/^\+/, '');
        ...
        if (phone_number !== (user.phone_number || '')) payload.phone_number = phone_number;
        if (company !== (user.company || '')) payload.company = company;
        if (country_code !== String(user.country_code || '')) payload.country_code = country_code;
        if (password) payload.password = password;

        // Always send email/phone when either changed so server can validate pair
        if (payload.email !== undefined || payload.phone_number !== undefined) {
            payload.email = email;
            ...
            <div className="bg-bg-primary w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl border border-border animate-slide-up">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-text-tertiary mt-0.5">
                                Profile & password only — permissions unchanged
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <form className="p-5 space-y-4" onSubmit={handleSubmit}>
                    ...
                    <p className="text-[10px] text-text-tertiary">
                        At least one of email or phone must remain set after save.
                    </p>
