...
import { LayoutGrid, List, Calendar, MapPin, Clock, Save, RefreshCw } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const handleRefreshEvents = async () => {
        ...
        try {
            ...
            console.error('Failed to refresh Snapcard events:', error);
        } finally {
    ...
    return (
        <div className="max-w-[1200px] mx-auto animate-fade-in">
            <div className="flex justify-between items-end mb-8 pb-4 border-b border-border">
                <div>
                    <span className="text-xs font-bold text-accent uppercase tracking-wider bg-accent/10 px-2 py-0.5 rounded-full mb-2 inline-block">Snapcard Mode</span>
                    <h1 className="text-2xl font-bold text-text-primary mb-1">Snapcard Events Overview</h1>
                    <p className="text-sm text-text-secondary">Select a Snapcard event to manage</p>
                </div>
            ...
            {events.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 px-8 text-text-tertiary text-center bg-bg-primary rounded-lg border border-dashed border-border">
                    ...
                    <h3>No Snapcard Events Found</h3>
                    <p>Click Refresh or check your credentials if you don't see any events.</p>
                </div>
            ) : (
